...
local function load_page(name, player) --> text, allow_save
	local text, allow_save = wikilib.plugin_handle_load(name, player)
	if text then
		return text, allow_save
	end
...
function wikilib.get_wiki_page_formspec(player, name)
	...
	local text, allow_save = load_page(name, player)

	local toolbar = (allow_save
			and "button[-.1,9.2;2.4,1;edit;Edit]"
			or "button[-.1,9.2;2.4,1;edit;View Source]")
	...
	return ("size[16,10]"
		...
		.. md2f.md2f(0.1, 1.1, 12, 9, text, "text")
		.. toolbar
	)
end

function wikilib.edit_wiki_page_formspec(player, name)
	if name == "" then name = "Main" end
...
function wikilib.edit_wiki_page(player, name)
	local fs = wikilib.edit_wiki_page_formspec(player, name)
	minetest.show_formspec(player, WIKI_FORMNAME, fs)
end
...
function wikilib.handle_formspec(player, formname, fields)
	...
	elseif fields.edit then
		wikilib.edit_wiki_page(plname, fields.page)
		return true
